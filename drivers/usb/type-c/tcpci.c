...
static int tcpci_set_pd_header(struct tcpc_dev *tcpc, enum typec_pwr_role pwr,
...
static irqreturn_t tcpci_irq(int irq, void *dev_id)
{
	...
	if (status & TCPC_ALERT_CC_STATUS) {
		...
		regmap_read(tcpci->regmap, TCPC_CC_STATUS, &reg);
		...
		tcpm_cc_change(tcpci->port, cc1, cc2);
	}
	...
	if (status & TCPC_ALERT_POWER_STATUS) {
		...
		if (reg == 0xff) {
			...
			if (reg & TCPC_POWER_STATUS_VBUS_PRES)
				tcpm_vbus_on(tcpci->port);
			else
				tcpm_vbus_off(tcpci->port);
		}
	}
